...
#  before_filter :correct_user, only: :update
  ...
  def show
    ...
    return redirect_to error_404_path unless @handle
  ...
  def update
    ...
    if params[:handle][:username].blank? || params[:handle][:username].length > 17
    ...
    redirect_to settings_path(current_user.email.split('@')[0])
  end
