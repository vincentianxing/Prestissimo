class Interact < ActionMailer::Base
  ...
  def notify_professor(course, email)
    ...
    important_fields = %w[building room start_time end_time days descrip dean_consent
                          instruct_consent status cancelled prof_desc which_desc prof_note display_prof_note]
    diff_fields.each do |f|
      next unless important_fields.include?(f)
      ...
      when 'status', 'cancelled'
        notify_fields['Status'] = case course.status
                                  when 'cancelled'
                                    'Cancelled'
                                  when 'hidden'
                                    'Hidden from search'
                                  else
                                    'Course displaying as normal (not hidden or cancelled)'
                                  end
      when 'descrip'
      ...
      when 'which_desc'
        notify_fields['Displayed description'] = case course.which_desc
                                                 when 'both'
                                                   'Displaying both discription from CIT and discription from faculty'
                                                 when 'professor'
                                                   'Displaying discription from faculty'
                                                 else
                                                   'Displaying description from CIT'
                                                 end
      when 'prof_note'
      ...
      when 'dean_consent', 'instruct_consent'
        notify_fields['Consent'] = if diff_fields.include?(dean_consent) && diff_fields.include?(instruct_consent)
                                     'Requires both consent of dean and consent of instructor'
                                   elsif diff_fields.include?(dean_consent)
                                     'Requires consent of instructor'
                                   else
                                     'Requires consent of dean'
                                   end
      else
        next
      end
